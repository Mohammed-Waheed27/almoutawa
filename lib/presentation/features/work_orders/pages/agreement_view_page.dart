...
import '../sections/agreement_view_header_section.dart';
import '../sections/agreement_view_lines_section.dart';
import '../sections/agreement_view_totals_section.dart';
import '../widgets/document_pdf_actions_bar.dart';
import '../widgets/document_view_skeleton.dart';

/// Read-only in-app agreement viewer with separate PDF actions.
class AgreementViewPage extends StatelessWidget {
    ...
    return RoleDashboardScaffold(
      ...
      body: BlocConsumer<WorkOrderDetailBloc, WorkOrderDetailState>(
        ...
        builder: (context, state) {
          ...
          final lineNotes = agreement == null
              ? ''
              : AgreementViewLinesSection.aggregatedNotes(agreement.lines);
          final pad = dense
          ...
          if (agreement == null) {
            return ListView(
              ...
              children: [
                Text('لا توجد اتفاقية محفوظة بعد.', textAlign: TextAlign.right),
                SizedBox(height: dense ? DesktopUiTokens.gapMd : AppSpacing.md),
                AlmoutawaButton(
                  label: 'إنشاء الاتفاقية',
                  expanded: !dense,
          ...
          return ListView(
            padding: EdgeInsets.all(pad),
            children: [
              AgreementViewHeaderSection(detail: detail),
              SizedBox(height: gap),
              InfoPanelSection(
                title: 'بيانات العميل',
                compact: true,
              ...
              SizedBox(height: gap),
              AgreementViewLinesSection(lines: agreement.lines),
              SizedBox(height: gap),
              AgreementViewTotalsSection(agreement: agreement),
              if (lineNotes.isNotEmpty) ...[
                SizedBox(height: gap),
                ExpandableNotesCard(title: 'ملاحظات البنود', body: lineNotes),
              ],
              SizedBox(height: dense ? DesktopUiTokens.gapMd : AppSpacing.md),
              Align(
                alignment: Alignment.centerRight,
                child: AlmoutawaButton(
                  label: 'تعديل الاتفاقية',
              ...
              DocumentPdfActionsBar(
                ...
                onAction: (action) => context.read<WorkOrderDetailBloc>().add(
                  WorkOrderDetailAgreementPdfRequested(
                    action,
                    context: context,
                  ),
                ),
